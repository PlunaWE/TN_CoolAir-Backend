...
async def _parse_request_payload(request: Request) -> dict:
    ...
    if "application/x-www-form-urlencoded" in content_type or "multipart/form-data" in content_type:
        try:
            ...
            # python-multipart missing: do not crash the return route, just fall back
            return data
        except Exception:
            return data
...
def list_products(db: Session = Depends(get_db)):
    ...
    for p in products:
        out.append(ProductOut(
            ...
            offers=[
                OfferOut(key="stromvorteil", name="Midea Portasplit 3,5 kW + Strom-Vorteil", price=p.offer_stromvorteil_price, badge="LAUNCH-ANGEBOT", description="150 Euro Wien Energie-Stromgutschein inklusive"),
                OfferOut(key="solo", name="Midea Portasplit 3,5 kW ohne Strom-Vorteil", price=p.offer_solo_price),
            ],
        ))
    return out
...
def add_cart_item(payload: AddCartItemIn, guest_id: str = Depends(guest_id_dep), db: Session = Depends(get_db)):
    data = CartService(db).add_item(guest_id, payload)
    return CartOut(**data)


@router.patch("/cart/items/{item_id}", response_model=CartOut)
def update_cart_item(item_id: str, payload: UpdateCartItemIn, guest_id: str = Depends(guest_id_dep), db: Session = Depends(get_db)):
    data = CartService(db).update_item(guest_id, item_id, payload.quantity)
    return CartOut(**data)


@router.delete("/cart/items/{item_id}", response_model=CartOut)
def delete_cart_item(item_id: str, guest_id: str = Depends(guest_id_dep), db: Session = Depends(get_db)):
...
def start_checkout(payload: CheckoutIn, guest_id: str = Depends(guest_id_dep), db: Session = Depends(get_db)):
...
async def telecash_connect_return(request: Request, result: str | None = None, db: Session = Depends(get_db)):
    payload = await _parse_request_payload(request)
    order_id = payload.get("oid") or payload.get("merchantTransactionId") or payload.get("orderId")
    if not order_id:
        return RedirectResponse(_frontend_payment_url("failure"), status_code=303)

    service = TeleCashService()
    if payload.get("response_hash"):
        payload["_response_hash_valid"] = service.verify_response_hash(payload, notification=False)
    payload["_result_hint"] = result or ""

    order, items = OrderService(db).apply_return(order_id, payload)
    target = "failure"
...
async def telecash_notify(request: Request, db: Session = Depends(get_db)):
    ...
    service = TeleCashService()
    if payload.get("notification_hash"):
        payload["_notification_hash_valid"] = service.verify_response_hash(payload, notification=True)
    order, items = OrderService(db).apply_return(order_id, payload)
    return {"ok": True, "order": serialize_order(order, items)}
